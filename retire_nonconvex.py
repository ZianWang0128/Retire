...
class low_dim():
    ...
    def fit(self, tau=0.5, robust=None, scale_invariant=True, \
        ...
        return {'beta': beta, 'res': res, \
                ...
                'c0': c0, 'lowerboundedcount': lbcount, 'multiplier': multiplier}  
...
class high_dim(low_dim):
    ...
    penalties = {'L1', 'SCAD', 'MCP', 'CappedL1'}
    ...
    def lambda_seq(self, nlambda=50, eps=0.01, standardize=True):
        ...
        return np.exp(np.linspace(np.log(eps*lambda_max), np.log(0.25*lambda_max), num=nlambda)) #may adjust the lower/upper bound  
    ...
    def irw(self, tau=0.5, Lambda=np.array([]), robust=False, scale_invariant=True, \
        ...
        c0_seq, lowerboundedcount_seq, robust_seq, count_seq = np.zeros(nstep+1), np.zeros(nstep+1),np.zeros(nstep+1),np.zeros(nstep+1)
        c0_seq[0], lowerboundedcount_seq[0], robust_seq[0], count_seq[0]  = model['c0'], model['lowerboundedcount'], model['robust'], model['count']
        ...
        while max_dev > self.opt['irw_tol'] and step <= nstep:
            ...
            model = self.l1(tau, rw_lam, robust, scale_invariant, \
                            model['beta'], model['res'], standardize, adjust=False, multiplier = multiplier)
            max_dev = max(abs(model['beta'] - beta))
            beta, res = model['beta'], model['res']

            c0_seq[step], lowerboundedcount_seq[step], robust_seq[step], count_seq[step]  = model['c0'], model['lowerboundedcount'], model['robust'], model['count']
        ...
        return {'beta': beta, 'res': res, 
                ...
                'robust': model['robust'],
                'nstep': step,
                'c0_seq': c0_seq,"lowerboundedcount_seq": lowerboundedcount_seq, "robust_seq": robust_seq, 'multiplier': multiplier, 'count':count_seq,'phi':last_phi } 
    ...
    def irw_path(self, tau=0.5, lambda_seq=np.array([]), \
                 ...
                 penalty="SCAD", a=3.7, nstep=3, standardize=True, adjust=True, multiplier=0.1):
        '''
        ...
        c0_seq, lowerboundedcount_seq = np.zeros(nlambda), np.zeros(nlambda)
        c0_seq[0], lowerboundedcount_seq[0]  = model['c0_seq'][0], model['lowerboundedcount_seq'].sum()
          

        for l in range(1, nlambda):
            model = self.irw(tau, lambda_seq[l], robust, scale_invariant, \
                             beta_seq[:,l-1], res_seq[:,l-1], \
                             penalty, a, nstep, standardize, adjust=False, multiplier = multiplier)
            beta_seq[:,l], res_seq[:,l] = model['beta'], model['res']
            robust_seq[l] = model['robust']

            c0_seq[l], lowerboundedcount_seq[l]  = model['c0_seq'][0], model['lowerboundedcount_seq'].sum()
    ...
    def fit(self, tau=0.5, lambda_seq=np.array([]), nlambda=50, \
        ...
        for v in range(nfolds):
            ...
            if penalty == "L1":
                model = train.l1_path(tau, lambda_seq, robust, scale_invariant, \
                                      standardize, adjust, multiplier = multiplier)
            else:
                model = train.irw_path(tau, lambda_seq, robust, scale_invariant, \
                                       penalty, a, nstep, standardize, adjust, multiplier = multiplier)
        ...
        if CV_rule == 'both' or CV_rule == 'min':
            ...
            if penalty == "L1":
                ...
                cv_model = rgs.irw(tau, lambda_min, robust, scale_invariant, \
                                   penalty=penalty, a=a, nstep=nstep, \
                                   standardize=standardize, adjust=adjust, multiplier = multiplier)
                c0_final,lowerboundedcount_final = cv_model['c0_seq'],cv_model['lowerboundedcount_seq']      
        ...
        if CV_rule == 'both' or CV_rule == '1se':        
            ...
            if penalty == "L1":
                ...
                cv_model = rgs.irw(tau, lambda_1se, robust, scale_invariant, \
                                   penalty=penalty, a=a, nstep=nstep, \
                                   standardize=standardize, adjust=adjust, multiplier = multiplier)
                c0_final,lowerboundedcount_final = cv_model['c0_seq'],cv_model['lowerboundedcount_seq']      
