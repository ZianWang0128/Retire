...
class low_dim():
    ...
    def _asym(self, x, tau):
        return  np.where(x < 0, (1-tau) * x, tau * x)   
    ...
    def fit(self, tau=0.5, robust=None, scale_invariant=True, \
            ...
            standardize=True, adjust=True, multiplier=0.1):
        '''
            Robust/Huberized Expectile Regression
        ...
        multiplier : the lowerbounding threshold. 0.1 is artificial, may tune this value. We use the heuristic approach 
                     robust = mad(asym_res)*sqrt{n/(log np)} to update the robustification parameter (robust). Multiplier ensures
                     that robust >= multiplier*initial robust, thus the robustification parameter can not be too small. Otherwise,
                     a very small robustification parameter leads to penalized quantile regressions, which ignore the benefit 
                     of strongly convex l2 loss, and will be insensitive to small covariates.
        ...
        if robust != None and scale_invariant:
            ...
            c = max( robust * self.mad(self._asym(res, tau)) , 
                     multiplier * c0 )
            if c == multiplier*c0: lbcount = lbcount+1 
        ...
        return {'beta': beta, 'res': res, \
                ...
                'c0': c0, 'lowerboundedcount': lbcount, 'multiplier': multiplier, 'tau':tau}  
...
class high_dim(low_dim):
    ...
    '''
    opt = {'phi': 0.01, 'gamma': 1.25, 'max_iter': 1e3, \
    ...
    def lambda_seq(self, nlambda=50, eps=0.01, standardize=True):
        ...
        return np.exp(np.linspace(np.log(0.5*eps*lambda_max), np.log(0.2*lambda_max), num=nlambda)) #may adjust the lower/upper bound  
    ...
    def _LAMM_update(self, beta0=np.array([]), gradient=np.array([]), phi=1.2, Lambda=np.array([]), alpha=0.5, w_vec=np.array([]), structure=np.array([]),
                     problem_type = "L1"): 
        '''
            LAMM update for various convex penalties

        Arguments
        ---------
        beta0 : the (k-1)-th iterate beta.
        
        gradient : the (k-1)-th iterate gradient of the loss function.
        
        phi : Parameter to ensure local majorization.

        Lambda : regularization parameter. This should be either a scalar or a vector.
        
        alpha : hybrid parameter for elastic net penalty. alpha = 0 amounts to ridge regression, alpha = 1 amounts to Lasso.
        
        w_vec : Vector of parameters used in Group Lasso or Sparse Group Lasso penalties
        
        structure : structures of the Group Lasso or Sparse Group Lasso penalties.
        
        problem_type : types of convex penalties, values are {'L1', 'EN', 'GL', 'SGL'}. 
                        'L1': weighted lasso;  'EN': elastic net;  'GL': group lasso;  'SGL': sparse group lasso
        

        Returns
        -------
        'beta_out' : a numpy array of the k-th iterate beta.
        
        '''
        if problem_type == 'L1':
            beta1 = beta0 - gradient/phi
            beta1[self.itcp:] = self._soft_thresh(beta1[self.itcp:], Lambda/phi)
            beta_out = beta1
        
        elif problem_type == 'EN':
            if Lambda.size != 1:
                raise ValueError("Lambda must be a scale value") 
            beta1 = beta0 - gradient/phi
            beta1[self.itcp:] = self._soft_thresh(beta1[self.itcp:], alpha*Lambda/phi)/(1+2*Lambda*(1-alpha)/phi)
            beta_out = beta1
        
        elif problem_type == "GL":
            if Lambda.size != 1:
                raise ValueError("Lambda must be a scale value") 
            if structure.size == 0:
                raise ValueError("structure must NOT be empty") 
            if w_vec.size != structure.size:
                raise ValueError("w_vec must match structure")                 
            beta1 = beta0 - gradient/phi            
            beta_temp = beta1[self.itcp:].copy()
            for idx,val in enumerate(structure.cumsum()):
                if idx == 0:
                    prev = 0 
                temp = beta_temp[prev:val]
                const = max( ( 1 - Lambda*w_vec[idx]/(phi* np.sqrt(sum(temp**2))) ), 0 )
                beta_temp[prev:val] = temp * const                           
                prev = val               
            beta1[self.itcp:] = beta_temp
            beta_out = beta1
        
        elif problem_type == "SGL":
            if Lambda.size != 1:
                raise ValueError("Lambda must be a scale value") 
            if structure.size == 0:
                raise ValueError("structure must NOT be empty") 
            ...
            for idx,val in enumerate(structure.cumsum()):
                if idx == 0:
                    prev = 0 
                temp = beta_temp[prev:val]
                temp2 = self._soft_thresh(temp, Lambda/phi)
                if sum(temp2**2) == 0: #avoid the case of dividing by zero
                    beta_temp[prev:val] = 0
                else:
                    const = max( ( 1 - Lambda*w_vec[idx]/(phi* np.sqrt(sum(temp2**2))) ), 0 )
                    beta_temp[prev:val] = temp2 * const                                                
                prev = val               
            beta1[self.itcp:] = beta_temp
            beta_out = beta1
        
        else:
            raise ValueError("problem_type must be one of L1, EN, GL, SGL")   
    ...
    def l1(self, tau=0.5, Lambda=np.array([]), robust=False, scale_invariant=True,
           beta0=np.array([]), res=np.array([]), standardize=True, adjust=True, multiplier=0.1,
           alpha=0.5, w_vec=np.array([]), structure=np.array([]),
                            problem_type = "L1"): 
        '''
        ...
        while dev > self.opt['tol'] and count < self.opt['max_iter']:
            ...
            #update beta#
            beta1 = self._LAMM_update(beta0, grad0, phi, Lambda,
            ...
            last_phi = phi             
            beta0, phi = beta1, max(self.opt['phi'], phi/self.opt['gamma'])
            count += 1
        ...
        return {'beta': beta1, 'res': res, 
                ...
                'lambda': Lambda, 
                'robust': c,
                'c0': c0, 'lowerboundedcount': lbcount, 'multiplier': multiplier, 'count': count, 'phi':last_phi,
                'alpha': alpha, 'w_vec': w_vec, 'structure': structure, 'problem_type': problem_type}  


    def l1_path(self, tau=0.5, lambda_seq=np.array([]), robust=False, \
                scale_invariant=True, standardize=True, adjust=True, multiplier=0.1,
                alpha=0.5, w_vec=np.array([]), structure=np.array([]),
                                 problem_type = "L1"): 
        '''
        ...
        model = self.l1(tau, lambda_seq[0], robust, scale_invariant, \
                        standardize=standardize, adjust=False, multiplier = multiplier,
                        alpha = alpha, w_vec = w_vec, structure=structure, problem_type=problem_type )
        beta_seq[:,0], res_seq[:,0] = model['beta'], model['res']
        robust_seq[0] = model['robust']

        c0_seq, lowerboundedcount_seq = np.zeros(nlambda), np.zeros(nlambda)
        c0_seq[0], lowerboundedcount_seq[0]  = model['c0'], model['lowerboundedcount']
        
        for l in range(1, nlambda):
            model = self.l1(tau, lambda_seq[l], robust, scale_invariant, \
                            beta_seq[:,l-1], res_seq[:,l-1], \
                            standardize, adjust=False, multiplier = multiplier,
                            alpha = alpha, w_vec = w_vec, structure=structure, problem_type=problem_type)
            beta_seq[:,l], res_seq[:,l] = model['beta'], model['res']
            robust_seq[l] = model['robust']

            c0_seq[l], lowerboundedcount_seq[l]  = model['c0'], model['lowerboundedcount']
        ...
        return {'beta_seq': beta_seq, \
                ...
                'c0_seq': c0_seq, 'lowerboundedcount_seq': lowerboundedcount_seq, 'multiplier': multiplier,
                'alpha': alpha, 'w_vec': w_vec, 'structure': structure, 'problem_type': problem_type} 
...
class cv(high_dim):
    ...
    def fit(self, tau=0.5, lambda_seq=np.array([]), nlambda=50, \
            ...
            nfolds=5, 
            standardize=True, adjust=True, multiplier = 0.1, CV_plot = False, CV_rule='both',
            alpha=0.5, w_vec=np.array([]), structure=np.array([]), problem_type = "L1"):  
        '''
        multiplier : default = 0.1, the lower bound of heuristic robust parameter. It can not be too small otherwise it will be indeed a quantile regression instead of a retire regression
        
        CV_plot : default = False. Make a CV curve: lambda sequence vs CV MSE.
        
        CV_rule : default = 1se. Other values: 'min', 'both'. The rule of selecting lambda
        ...
        c0_seq, lowerboundedcount_mat, robust_mat = np.zeros(nfolds), np.zeros((nfolds, nlambda)), np.zeros((nfolds, nlambda))
        ...
        for v in range(nfolds):
            ...
            model = train.l1_path(tau, lambda_seq, robust, scale_invariant, \
                                      standardize, adjust, multiplier, alpha, w_vec, structure, problem_type)

            c0_seq[v], lowerboundedcount_mat[v,:], robust_mat[v,:]  = model['c0_seq'][0], model['lowerboundedcount_seq'], model['robust_seq']
        ...
        cv_se = np.std(val_err, axis = 0) / np.sqrt(nfolds)

        index_min = np.argmin(cv_err)
        cutoff = cv_err[index_min] + cv_se[index_min]
        index_1se = np.where(cv_err <= cutoff)[0].max()
        
        cv_err_min = cv_err[index_min]
        cv_err_1se = cv_err[index_1se]
        lambda_min = model['lambda_seq'][index_min]
        lambda_1se = model['lambda_seq'][index_1se]
        
        if CV_plot:
            plt.plot(model['lambda_seq'], cv_err)
            plt.fill_between(model['lambda_seq'], cv_err + 1*cv_se, cv_err - 1*cv_se, alpha=.1)
            plt.axhline(cutoff, linestyle='dotted', label='Best + One SE')
            plt.scatter([model['lambda_seq'][index_1se]], [cv_err[index_1se]], marker='o', color='orange', label='One SE Rule')
            plt.scatter([model['lambda_seq'][index_min]], [cv_err[index_min]], marker='o', color='blue', label='Minimum rule')
            plt.legend()
            plt.xlabel('Log lambda sequence')
            plt.ylabel('CV MSE')
            plt.xscale('log')
            plt.show()
        
        advanced_cv_stage_results = {'c0_seq': c0_seq, 'lowerboundedcount_mat': lowerboundedcount_mat, 
                                     'robust_mat':robust_mat,'multiplier': multiplier,
                                     'val_err':val_err, 'cv_err': cv_err, 'tau': tau}
        
        size_min = size_1se=np.nan
        fit_result_min = fit_result_1se=np.nan
        
        if CV_rule == 'both' or CV_rule == 'min':
            ###lambda_min###

            cv_model = rgs.l1(tau, lambda_min, robust, scale_invariant,\
                                  standardize=standardize, adjust=adjust, multiplier = multiplier,
                                  alpha = alpha, w_vec = w_vec, structure=structure, problem_type=problem_type)
            c0_final,lowerboundedcount_final = cv_model['c0'],cv_model['lowerboundedcount']      
            
            fit_result_min = {'cv_beta': cv_model['beta'],'cv_res': cv_model['res'], 'robust': cv_model['robust'],
                              'c0_final': c0_final, 'lowerboundedcount_final': lowerboundedcount_final, "count":cv_model['count'], 'phi':cv_model['phi'] }
            size_min = np.sum(cv_model['beta']!=0, axis=0)
        
        if CV_rule == 'both' or CV_rule == '1se':        
            ###lambda_1se###

            cv_model = rgs.l1(tau, lambda_1se, robust, scale_invariant,\
                                  standardize=standardize, adjust=adjust, multiplier = multiplier,
                                  alpha = alpha, w_vec = w_vec, structure=structure, problem_type=problem_type)
            c0_final,lowerboundedcount_final = cv_model['c0'],cv_model['lowerboundedcount']      
            
            fit_result_1se = {'cv_beta': cv_model['beta'],'cv_res': cv_model['res'], 'robust': cv_model['robust'],
                              'c0_final': c0_final, 'lowerboundedcount_final': lowerboundedcount_final, "count":cv_model['count'], 'phi':cv_model['phi']  }
            size_1se = np.sum(cv_model['beta']!=0, axis=0)

        return {'lambda_seq': model['lambda_seq'], 
                'lambda_min': lambda_min, 'lambda_1se':lambda_1se,
                'index_min':index_min, 'index_1se':index_1se,
                'cv_err_min': cv_err_min, 'cv_err_1se':cv_err_1se,
                'size_min':size_min, 'size_1se' : size_1se,
                'advanced_cv_stage_results':advanced_cv_stage_results,
                'fit_result_min':fit_result_min,'fit_result_1se':fit_result_1se, 'CV_rule':CV_rule}    